...
external effect that an evaluator can check. That makes regressions visible and
keeps the runtime honest as new capabilities are added.

Milestone 0 proves the smallest complete story. The runtime should fetch one
document, produce a short summary, and post the result to a sink service while
recording trace events for every important transition.
